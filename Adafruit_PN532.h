...
public:
  ...
  // Custom function for 2 tags
  bool readTwoPassiveTargetIDs(uint8_t cardbaudrate, 
                               uint8_t *uid1, uint8_t *uid1Length, 
                               uint8_t *uid2, uint8_t *uid2Length);
  // Custom functions for 2 tags                             
  bool mifareclassic_AuthenticateBlockTarget(uint8_t targetNumber, uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData);
  bool mifareclassic_ReadDataBlockTarget(uint8_t targetNumber, uint32_t blockNumber, uint8_t *data);
  bool mifareultralight_ReadPageTarget(uint8_t targetNumber, uint8_t page, uint8_t *buffer);
  bool inRelease(uint8_t releaseNum = 0);
